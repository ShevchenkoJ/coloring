...
function cellClick(clickedCell) {
    const cellToGetBackground = clickedCell.target;
...
function fieldClick(clickedField) {
    if ( cellBackgroundProperty != undefined ) {
        // clickedField.target.style.background = cellBackgroundProperty;
        const fieldToSetBackground = clickedField.target;
...
function setColorProperty(coloredField, addedColor) {
    ...
    coloredField.style.background = addedColor;
